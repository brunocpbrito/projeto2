    ...
    if (aluno->getNome() == "turma") {
        ...
        if (pegarEspera) {
            if (turma.getLength() < capacidadeFila && !filaEspera.isEmpty()) {
                EV << "\n Turma com " << turma.getLength()  << " alunos, restando "   << (capacidadeFila - turma.getLength())  << " vagas. Pegando alunos da fila de espera ("   << filaEspera.getLength()   << ") do Periodo, ate completar as vagas. " << endl;
        ...
        EV << "\n Criando turma no Periodo de "<< turma.getLength() <<" alunos e fila de espera "<< filaEspera.getLength() <<" \n" << endl;
...
void SegundoPeriodo::colocarFila(Aluno *aluno) {
    ...
    if (turma.getLength() < capacidadeFila) {
        ...
        turma.insert(aluno);
        if (turma.getLength() == capacidadeFila) {
            EV << "\n Turma do Periodo com "<< turma.getLength() <<" completa, o resto vai para a fila de espera. "  << endl;
        }
    } else {
...
void SegundoPeriodo::finish(){
    EV << "\n ## VALORES PARA O PERIODO ##" << endl;
    EV << "Capacidade da turma de "<< capacidadeFila <<" alunos" << endl;
    EV << "Valores para a fila de espera do periodo" << endl;
    ...
    EV << "Valores para a turma do Periodo" << endl;
    ...
    EV << "Total de alunos que evadiram o periodo: " << filaCancelados.getLength() << endl;
}
...
void SegundoPeriodo::destinoAluno(Aluno *aluno) {
    ...
    if (probabilidade >= probCancelamentoDisciplina) {
        ...
        if (aluno->getNota() >= probReprovacao) {
            ...
            EV << "Aprovado aluno \"" << aluno->getNumero()   << "\" sendo enviado para Prox periodo " << endl;
        ...
        EV << "Aluno \"" << aluno->getNumero()   << "\" evadiu. Total de evadidos: "<< filaCancelados.getLength() + 1 << " " << endl;
    }
}
